...
class DockController:
    def __init__(self, rfid_dock_callback=None) -> None:
        ...
        if len(rfid_pins) != len(nums_pixels):
            ...
            for key, rfid_pin, num_pixels in zip(range(len(num_pixels)), rfid_pins, nums_pixels):
                dock = Dock(rfid_dock_callback)
                dock.set_rfid(rfid_pin)
                dock.set_ring_led(NeoLedPins.instance().pin_number, nums_pixels, starting_pixel=key * nums_pixels)
